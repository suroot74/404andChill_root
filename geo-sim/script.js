...
// Initialize placeholder (Ensure it is added only once)
...
function loadAssets(type, folder, container, items, isBackground = false) {
  ...
  if (items.length > maxVisible) {
    ...
    expandButton.classList.add("expand-toggle");
...
function createAssetElement(type, folder, container, file, isBackground) {
  ...
  if (isBackground) {
    ...
    element.draggable = true; // Make GIF draggable
...
function addCloseButton(element) {
  ...
  closeButton.addEventListener("click", () => {
    ...
    closeButton.remove(); // Remove the button itself
...
function showToolbar(element) {
  ...
  // Load current styles into the toolbar
...
function loadTemplate(template) {
  ...
  template.elements.forEach(el => {
    ...
    // Enable dragging and removal for the element
